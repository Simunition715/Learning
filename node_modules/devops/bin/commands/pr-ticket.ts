...
import url from "url";
import viewTicket from "./ticket-view.js";
...
async function openGitlabPR(
  ...
  const cachePath = await getPackageCachePath("puppeteer", "pr-ticket");
  ...
  await page.goto(
    `${repoUrl}/-/merge_requests/new?utf8=%E2%9C%93&merge_request%5Bsource_project_id%5D=${projectId}&merge_request%5Bsource_branch%5D=${encodeURIComponent(
      sourceBranch
    )}&merge_request%5Btarget_project_id%5D=${projectId}&merge_request%5Btarget_branch%5D=dev`
  );
  ...
  await page.evaluate(
    (ticketURL, sourceBranch) => {
      ...
      if (node) node.value = `PR: ${sourceBranch}`;
      node = document.querySelector<HTMLInputElement>(
        "#merge_request_description"
      );
      if (node) {
        node.value = `This is a PR request for the ticket: ${ticketURL}`;
      }
    },
    ticketURL,
    sourceBranch
...
export default async function run(
  ...
  try {
    ...
    const { shouldContinue } = await inquirer.prompt<{
    ...
    }>([
      {
        ...
        name: "shouldContinue",
        message:
          "There are no detected semver messages that will cause a version change. Do you want to continue?",
      },
    ]);
  ...
  if (!branch || !hash || !ticketUrl) {
    console.error(
      "Failed to retrieve necessary ticket information from current branch."
    );
    process.exit(1);
  }
